...
abstract class BaseBottomSheetFragment : BottomSheetDialogFragment() {
    ...
    final override fun onCreateDialog(savedInstanceState: Bundle?): Dialog = if (animateStatusBar()) {
        BaseSuperBottomSheetDialog(context, R.style.superBottomSheetDialog)

    } else BaseSuperBottomSheetDialog(context)
    ...
    private fun iniBottomSheetUiComponents() {
        ...
        sheetContainer.setBackgroundColor(   requireActivity().resources.getColor(R.color.transparent))
        ...
        callback = object : BottomSheetBehavior.BottomSheetCallback() {
            override fun onStateChanged(bottomSheet: View, newState: Int) {
                if (newState == BottomSheetBehavior.STATE_HIDDEN) {
                    ...
                    BottomSheetExpendObservable.updateBottomSheetState(true)
                }else if (newState == BottomSheetBehavior.STATE_COLLAPSED) {
                    BottomSheetExpendObservable.updateBottomSheetState(false)
                }
            }
    ...
    open fun getPeekHeight() = with(requireContext().getAttrId(R.attr.superBottomSheet_peekHeight)) {
        ...
        with(resources.displayMetrics) {
            peekHeightMin.coerceAtLeast(heightPixels - heightPixels * 1/4)
        }
    ...
    open fun getStatusBarColor() = with(requireContext().getAttrId(R.attr.superBottomSheet_statusBarColor)) {
        when (this) {
            INVALID_RESOURCE_ID -> ContextCompat.getColor(
                requireContext(),
                requireContext().getAttrId(R.attr.colorPrimaryDark)
            )
            else -> ContextCompat.getColor(requireContext(), this)
        }
    }
